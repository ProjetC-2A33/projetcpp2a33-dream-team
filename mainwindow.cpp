...
#include "ui_mainwindow.h" // FIX: Ensure this include exists
#include "projet.h"

#include <QDialog> // FIX: Changed "" to <>
#include <QWidget> // FIX: Added for PieWidget
#include <QFileDialog>
#include <QMessageBox>
#include <QPushButton>
...
MainWindow::MainWindow(QWidget *parent)
    : QDialog(parent), ui(new Ui::mainwindow) {  // Constructor matches the UI class
    ui->setupUi(this);
    ...
    // Input validation for Budget field
    ...
    // Wire buttons
    connect(ui->pushButton_60, &QPushButton::clicked, this, &MainWindow::onAdd);
    connect(ui->pushButton_61, &QPushButton::clicked, this, &MainWindow::onExportPdf);
    ...
    // Wire delete button
    if (ui->pushButton_59) {
        connect(ui->pushButton_59, &QPushButton::clicked, this, &MainWindow::onDelete);
    }
...
// ... [Rest of your functions: onApplyTableEdits, refreshTable, onAdd, onUpdate, onDelete, onExportPdf, onSortChanged, onSearchTextChanged, onSendEmail, onOpenChatAssistant] ...
// (These functions from your original code were logically fine, just ensure Projet class works)
...
void MainWindow::onUpdate() {
   // Implementation reused from your code...
...
void MainWindow::onOpenChatAssistant() {
    ...
    auto answerFromTable = [table](const QString &q) {
        ...
        // Simple parsing logic for the assistant
        ...
        // ... (Include the rest of your Chat Assistant logic here) ...
        // For brevity, I am assuming you keep the long logic block you provided in the original file.
        else {
             for (int r=0;r<rows;++r) {
    ...
    QObject::connect(send, &QPushButton::clicked, &dlg, [table, history, input, answerFromTable, this]() {
        ...
        if (ql.contains("sort") || ql.contains("trier")) {
             // Basic sorting triggers based on chat commands
            if (ql.contains("id") && ql.contains("asc")) onSortChanged(0);
            else if (ql.contains("id") && (ql.contains("desc"))) onSortChanged(1);
            // ... add other sort mappings ...
        }
        input->clear();
    });
...
{
    smartstudio *dg = new smartstudio(this);
    hide();
    dg->show();
}
