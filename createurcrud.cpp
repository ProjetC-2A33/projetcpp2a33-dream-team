...
// ==================== ARDUINO - RECHERCHE PAR ID ====================
QString createurcrud::rechercherNomPrenomParId(QString id)
{
    QSqlQuery query;
    query.prepare("SELECT nom, prenom FROM createur WHERE idc = :id");
    query.bindValue(":id", id);

    if (!query.exec()) {
        qDebug() << "Erreur recherche créateur par ID:" << query.lastError().text();
        return "";
    }
