...
bool Employe::ajouter() {
    ...
    // FIX: Convert float to string for Oracle ODBC
    QString salaireStr = QString::number(salaire, 'f', 2); // 2 decimal places
    query.bindValue(":sal", salaireStr);

    query.bindValue(":mail", e_mail);
    query.bindValue(":type", type_employe);

    qDebug() << "Executing query with values:";
    qDebug() << "ID:" << id_employe << "(int)";
    qDebug() << "Tel:" << num_tel << "(int)";
    qDebug() << "Salaire:" << salaireStr << "(string)";
    qDebug() << "Email:" << e_mail;
    qDebug() << "Type:" << type_employe;
