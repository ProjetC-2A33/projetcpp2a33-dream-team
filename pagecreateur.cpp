...
pagecreateur::pagecreateur(QWidget *parent)
    ...
    , arduinoBuffer("")
    , currentArduinoInput("")
    , arduinoSearchDialog(nullptr)
    , arduinoInputLabel(nullptr)
    , arduinoResultLabel(nullptr)
    , arduinoStatusLabel(nullptr)
{
    ...
    // Ouvrir automatiquement la fenêtre de recherche Arduino
    openArduinoSearchDialog();
...
{
    if (arduinoSearchDialog) {
        arduinoSearchDialog->close();
        delete arduinoSearchDialog;
    }
    if (A) {
        A->close_arduino();
        delete A;
...
// ==================== ARDUINO SETUP ====================
void pagecreateur::setupArduino()
{
    A = new arduino();
    
    // Essayer de connecter directement sur COM7 (votre port Arduino)
    QSerialPort *serial = A->getserial();
    
    // Liste des ports à essayer
    QStringList ports = {"COM7", "COM6", "COM3", "COM4", "COM5", "COM8"};
    bool connected = false;
    
    for (const QString &portName : ports) {
        qDebug() << "🔄 Tentative connexion clavier sur" << portName;
        ...
        if (serial->open(QSerialPort::ReadWrite)) {
            qDebug() << "✅ Arduino CLAVIER connecté sur" << portName;
            connected = true;
            
            // Attendre que Arduino soit prêt
            QThread::msleep(1500);
            serial->clear();
            
            sendToArduinoLCD("RECHERCHE", "Createur...");
            break;
        } else {
            qDebug() << "❌ Impossible d'ouvrir" << portName << ":" << serial->errorString();
        }
    }
    
    if (connected) {
        // Timer pour lire les données Arduino
        QTimer *timer = new QTimer(this);
        connect(timer, &QTimer::timeout, this, &pagecreateur::readArduinoData);
        timer->start(50);  // Lire toutes les 50ms
        qDebug() << "🎹 Clavier Arduino prêt - Tapez sur le clavier!";
    } else {
        qDebug() << "⚠️ Arduino CLAVIER non connecté - Port peut-être utilisé par autre chose";
        qDebug() << "💡 Fermez le moniteur série Arduino IDE ou autre application";
    }
}

// ==================== FENÊTRE DE RECHERCHE ARDUINO ====================
void pagecreateur::openArduinoSearchDialog()
{
    if (arduinoSearchDialog) {
        arduinoSearchDialog->show();
        arduinoSearchDialog->raise();
        return;
    }
    
    // Créer la fenêtre de dialogue
    arduinoSearchDialog = new QDialog(this);
    arduinoSearchDialog->setWindowTitle("🎹 Recherche Créateur - Clavier Arduino");
    arduinoSearchDialog->setFixedSize(450, 350);
    arduinoSearchDialog->setStyleSheet(
        "QDialog {"
    ...
    QVBoxLayout *layout = new QVBoxLayout(arduinoSearchDialog);
    layout->setSpacing(15);
    layout->setContentsMargins(20, 20, 20, 20);
    
    // Titre
    QLabel *titleLabel = new QLabel("🎹 RECHERCHE CRÉATEUR");
    ...
    QLabel *instructionsLabel = new QLabel(
        ...
        "• Tapez l'ID du créateur (0-9)\n"
        "• Appuyez sur # pour RECHERCHER\n"
        "• Appuyez sur * pour EFFACER"
    );
    instructionsLabel->setAlignment(Qt::AlignCenter);
    instructionsLabel->setStyleSheet(
        "font-size: 12px; color: #ffffff; padding: 10px;"
        "background: rgba(0,0,0,0.3); border-radius: 10px;"
    );
    layout->addWidget(instructionsLabel);
    
    // Status Arduino
    arduinoStatusLabel = new QLabel("⏳ Connexion Arduino...");
    arduinoStatusLabel->setAlignment(Qt::AlignCenter);
    arduinoStatusLabel->setStyleSheet("font-size: 12px; color: #ffff00;");
    layout->addWidget(arduinoStatusLabel);
    
    // Affichage de l'ID saisi
    QLabel *idTitleLabel = new QLabel("ID saisi:");
    idTitleLabel->setStyleSheet("font-size: 14px; color: #00ff00;");
    layout->addWidget(idTitleLabel);
    
    arduinoInputLabel = new QLabel("");
    arduinoInputLabel->setAlignment(Qt::AlignCenter);
    arduinoInputLabel->setStyleSheet(
        "font-size: 36px; font-weight: bold; color: #00ffff;"
        "background: rgba(0,0,0,0.5); border: 2px solid #00ffff;"
        "border-radius: 10px; padding: 15px; min-height: 50px;"
    );
    layout->addWidget(arduinoInputLabel);
    
    // Résultat de la recherche
    arduinoResultLabel = new QLabel("En attente de saisie...");
    arduinoResultLabel->setAlignment(Qt::AlignCenter);
    arduinoResultLabel->setWordWrap(true);
    arduinoResultLabel->setStyleSheet(
        "font-size: 14px; color: #ffffff;"
        "background: rgba(0,0,0,0.3); border-radius: 10px;"
        "padding: 15px; min-height: 60px;"
    );
    layout->addWidget(arduinoResultLabel);
    ...
    connect(closeButton, &QPushButton::clicked, arduinoSearchDialog, &QDialog::hide);
    layout->addWidget(closeButton);
    
    // Mettre à jour le status
    if (A && A->getserial()->isOpen()) {
        arduinoStatusLabel->setText("✅ Arduino connecté sur COM7");
        arduinoStatusLabel->setStyleSheet("font-size: 12px; color: #00ff00;");
    } else {
        arduinoStatusLabel->setText("❌ Arduino non connecté");
        arduinoStatusLabel->setStyleSheet("font-size: 12px; color: #ff0000;");
    }
    
    arduinoSearchDialog->show();
}

void pagecreateur::updateArduinoSearchDialog()
{
    if (arduinoInputLabel) {
        arduinoInputLabel->setText(currentArduinoInput);
    }
}

void pagecreateur::readArduinoData()
{
    if (!A) return;
    ...
    if (serial->bytesAvailable() > 0) {
        ...
        arduinoBuffer += QString::fromUtf8(data);
        
        qDebug() << "📥 Données reçues:" << data;
        
        // Traiter les lignes complètes
        while (arduinoBuffer.contains('\n')) {
            int index = arduinoBuffer.indexOf('\n');
            QString line = arduinoBuffer.left(index).trimmed();
            arduinoBuffer = arduinoBuffer.mid(index + 1);
            
            if (!line.isEmpty()) {
                qDebug() << "🎹 Arduino Clavier:" << line;
                
                // Traiter les touches
                if (line.startsWith("KEY:")) {
                    QString key = line.mid(4).trimmed();
                    processArduinoKey(key);
                }
                else if (line.startsWith("PASSWORD:")) {
                    // ID complet saisi, rechercher le créateur
                    QString id = line.mid(9).trimmed();
                    searchAndDisplayCreateur(id);
                }
            }
        }
    }
...
void pagecreateur::processArduinoKey(const QString &key)
{
    if (key == "#") {
        // Valider - Rechercher le créateur avec l'ID actuel
        if (!currentArduinoInput.isEmpty()) {
            searchAndDisplayCreateur(currentArduinoInput);
        } else {
            if (arduinoResultLabel) {
                arduinoResultLabel->setText("⚠️ Veuillez saisir un ID!");
                arduinoResultLabel->setStyleSheet(
                    "font-size: 14px; color: #ffff00;"
                    "background: rgba(255,255,0,0.2); border-radius: 10px;"
                    "padding: 15px; min-height: 60px;"
                );
            }
        }
        ...
        currentArduinoInput = "";
        ui->l2_15->clear();
        updateArduinoSearchDialog();
        sendToArduinoLCD("Efface", "");
        
        if (arduinoResultLabel) {
            arduinoResultLabel->setText("Saisie effacée. Entrez un nouvel ID.");
            arduinoResultLabel->setStyleSheet(
                "font-size: 14px; color: #ffffff;"
                "background: rgba(0,0,0,0.3); border-radius: 10px;"
                "padding: 15px; min-height: 60px;"
        ...
        ui->l2_15->clear();
        updateArduinoSearchDialog();
        sendToArduinoLCD("Annule", "");
    }
    else if (key.length() == 1 && (key[0].isDigit() || key[0].isLetter())) {
        // Ajouter le caractère à la saisie
        if (currentArduinoInput.length() < 10) {
            currentArduinoInput += key;
            ui->l2_15->setText(currentArduinoInput);
            updateArduinoSearchDialog();
            
            // Afficher sur LCD
            sendToArduinoLCD("ID: " + currentArduinoInput, "# = Rechercher");
        }
    }
}

void pagecreateur::searchAndDisplayCreateur(const QString &id)
{
    qDebug() << "🔍 Recherche créateur ID:" << id;
    
    // Mettre l'ID dans le champ de recherche
    ui->l2_15->setText(id);
    
    // Afficher "Recherche en cours" dans la fenêtre
    if (arduinoResultLabel) {
        arduinoResultLabel->setText("🔍 Recherche en cours...");
        arduinoResultLabel->setStyleSheet(
            "font-size: 14px; color: #ffff00;"
            "background: rgba(0,0,0,0.3); border-radius: 10px;"
            "padding: 15px; min-height: 60px;"
    ...
    QSqlQuery query;
    query.prepare("SELECT IDC, NOM, PRENOM, EMAIL, PLATFORME, NUMTEL, NBABONNES FROM CREATEUR WHERE IDC = :id");
    ...
    if (query.exec() && query.next()) {
        ...
        QString platforme = query.value(4).toString();
        QString numtel = query.value(5).toString();
        QString nbabonnes = query.value(6).toString();
        
        qDebug() << "✅ Créateur trouvé:" << nom << prenom;
        
        // Afficher sur LCD Arduino
        QString ligne1 = nom + " " + prenom;
        QString ligne2 = platforme + " " + nbabonnes + " abos";
        sendToArduinoLCD(ligne1, ligne2);
        
        // Afficher dans la fenêtre Qt
        if (arduinoResultLabel) {
            QString resultText = QString(
                ...
                "🎬 Plateforme: %5\n"
                "👥 Abonnés: %6"
            ).arg(nom, prenom, email, numtel, platforme, nbabonnes);
            
            arduinoResultLabel->setText(resultText);
            arduinoResultLabel->setStyleSheet(
        ...
        // Rafraîchir le tableau avec la recherche
        ui->tableView->setModel(crc.rechercherParId(id));
        
        // Reset la saisie pour la prochaine recherche
        currentArduinoInput = "";
        updateArduinoSearchDialog();
        
    } else {
        qDebug() << "❌ Créateur non trouvé pour ID:" << id;
        sendToArduinoLCD("NON TROUVE!", "ID: " + id);
        ...
        if (arduinoResultLabel) {
            arduinoResultLabel->setText(
                ...
                "Appuyez sur * pour effacer et réessayer."
            );
            arduinoResultLabel->setStyleSheet(
...
void pagecreateur::sendToArduinoLCD(const QString &line1, const QString &line2)
{
    if (!A || !A->getserial()->isOpen()) return;
    
    QString command;
    if (line2.isEmpty()) {
        command = "MSG:" + line1.left(16) + "\n";
    } else {
        command = "DISPLAY:" + line1.left(16) + "|" + line2.left(16) + "\n";
    }
    
    A->write_to_arduino(command.toUtf8());
    qDebug() << "📤 Envoyé au LCD:" << command.trimmed();
}
