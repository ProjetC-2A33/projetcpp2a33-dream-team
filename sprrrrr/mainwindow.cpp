...
#include <QPainter>
#include <QDate>
#include <QRegularExpression>// Pour vérifier téléphone
#include <QListView>
...
#include <QInputDialog>//Modifier le texte avant d’envoyer le SMS
#include <QBuffer>
#include <QMessageBox>
...
MainWindow::MainWindow(QWidget *parent)
    ...
    TCP_Server(new QTcpServer(this)),//Création d’un serveur TCP (pour la communication réseau)
    networkManager(new QNetworkAccessManager(this))//creation du sms via api
{
    ui->setupUi(this);
    ...
    connect(ui->stat, &QPushButton::clicked,
            this, &MainWindow::afficherStatistiquesCirculaire);

    chargerSponsors();//on charge tous les sponsors depuis la base de données
    TCP_Server = new QTcpServer();//attendre des connexions venant d’autres clients
    if(TCP_Server->listen(QHostAddress::Any, 8889)) {//accepte toutes les connexions
        connect(TCP_Server, &QTcpServer::newConnection, this, &MainWindow::newConnection);
        QMessageBox::information(this,"Qt","Server Started on port 8889");//Quand un nouveau client se connecte, la fonction newConnection()
    } else {
        QMessageBox::critical(this,"Qt","Failed to start server on port 8889");
    }
    ...
    connect(networkManager, &QNetworkAccessManager::finished,
            this, &MainWindow::onSMSReply);//Après l’envoi du SMS, quand le site Infobip répond,la fonction affiche si true or false
...
bool MainWindow::controleSaisie()
{
    ...
    QString email   = ui->lineEdit_email->text().trimmed();//on enlève les espaces inutiles avec trimed
...
void MainWindow::chargerSponsors()
{
...
void MainWindow::ajouterSponsor()
{
    if (!controleSaisie())
        return;
...
void MainWindow::modifierSponsor()
{
...
void MainWindow::supprimerSponsor()
{
    int row = ui->tableWidget_sponsors->currentRow();
    if (row < 0 || row >= sponsors.size()) {
...
void MainWindow::appliquerRecherche(const QString &texte)
{
...
void MainWindow::exporterPdf()
{
    QString fileName = QFileDialog::getSaveFileName(
        ...
        "sponsors.pdf",
        "Fichiers PDF (*.pdf)");//nom du fichier doss de sauv
    if (fileName.isEmpty())
        return;
    ...
    // On garde le ratio du tableau, on le scale au max sans dépasser la page
    ...
    // Décalages pour centrer horizontalement MAIS rester en haut verticalement
    double offsetX = pageRect.x() + (pageRect.width()  - tableDrawWidth) / 2.0;
    double offsetY = pageRect.y() + 40;   // Tableau en haut
...
void MainWindow::Read_Data_From_Socket()
{
    ...
    if (socket->bytesAvailable() > 0) {
        ...
        QString Message = "Employe : " + QString::number(socket->socketDescriptor()) + " :: " + QString(Message_From_Server);
...
void MainWindow::Add_New_Client_Connection(QTcpSocket *socket)
{
    ...
    QString Client = "Admin : " + QString::number(socket->socketDescriptor()) + " Connected With The Server.";
...
void MainWindow::envoyerSMS(const QString& telephone, const QString& message)
{
    ...
    // Create CORRECT JSON payload for Infobip Advanced SMS API
    ...
    // For free trial, use Infobip's test sender - this is REQUIRED
    ...
    // Remove this line - Qt handles Content-Length automatically
...
void MainWindow::onSMSReply(QNetworkReply *reply)
{
    if (reply->error() == QNetworkReply::NoError) {
        ...
        // Infobip success response structure
        ...
        // Try to parse error response from Infobip
        if (!response.isEmpty()) {
...
void MainWindow::on_pushButton_envoyerSMS_clicked()
{
    ...
    // Show dialog to customize message
    bool ok;
...
void MainWindow::on_Openhercules_clicked()
{
